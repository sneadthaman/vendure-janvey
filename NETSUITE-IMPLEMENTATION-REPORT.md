...
### Contact invitation onboarding (2026-09-23)

Staff can send an invitation from **Customers > NetSuite customers** using an imported account and immutable NetSuite contact ID. Before sending, the backend fetches the live Customer Master, requires an active contact with an email address, validates any selected default ship-to, revokes older pending invitations for that contact, and stores only a SHA-256 hash of a random single-use token. Invitations expire after seven days by default; `NETSUITE_INVITATION_TTL_HOURS` can override the duration.

The recipient opens `/account-invitation`, then signs in or uses the existing registration and email-verification flow with the invited address. Acceptance requires an authenticated verified Vendure user whose normalized email matches the invitation. The backend rechecks the live NetSuite account, contact, and email immediately before creating the contact link. The invitation's approval role and default ship-to are applied atomically, and accepted, expired, or revoked tokens cannot be replayed.

A local end-to-end run used an unused active contact on customer `712`: the development email rendered with the invitation link, public preview succeeded, a verified matching temporary login accepted it, `PROF MAINTENANCE OF LI` became the active account with `requiresApproval=true`, and token replay was rejected. The temporary customer, link, and invitation were removed after the test.

### Production email configuration (2026-09-24)

The EmailPlugin uses its local file mailbox when `EMAIL_TRANSPORT=file`, which is the development default. `EMAIL_TRANSPORT=microsoft-graph` sends full MIME messages through Microsoft Graph using the OAuth client-credentials flow, caches access tokens, and retries once after an authorization expiry. This avoids Exchange Online Basic authentication and mailbox app passwords. Graph delivery requires `EMAIL_FROM_ADDRESS`, `M365_TENANT_ID`, `M365_CLIENT_ID`, `M365_CLIENT_SECRET`, and `M365_SENDER_MAILBOX`; non-development environments also require `STOREFRONT_URL`. Verification, password-reset, email-change, approval, order, and contact-invitation links all use the configured storefront origin. Safe placeholders and the complete backend environment contract are documented in `apps/server/.env.example`; real credentials remain untracked. `apps/server/MICROSOFT-365-EMAIL.md` documents the required Entra `Mail.Send` application permission, administrator consent, and mailbox scoping.

The invitation form no longer asks staff to copy raw NetSuite contact IDs or local address IDs. Selecting an imported eligible account fetches its current live contacts, filters to active contacts with email addresses, and presents those contacts alongside the account's active imported ship-to addresses. The backend still performs the same authoritative live validation when the invitation is sent.

Microsoft Graph accepted the first external delivery test on 2026-09-24 at `17:32:41Z`, sending from and to `sjanvey@janvey.com`, and the message arrived in the mailbox. This validated the configured tenant, application permission, client credential, token request, sender mailbox, MIME generation, Graph `sendMail` request, and final delivery.

The rendered contact-invitation path was then exercised against live NetSuite customer `712`. NetSuite returned the newly added active test contact `5847` (`sjanvey@janvey.com`), the Admin API created invitation `3`, and Vendure email job `2093` completed on its first attempt. The delivered message exposed the untouched Vendure scaffold header/footer placeholders and malformed quoted-printable rendering caused by LF-only MIME line endings. The shared template now identifies Janvey, the invitation uses an email-safe button plus a visible fallback URL, and the Graph sender generates CRLF MIME. A regression test verifies CRLF and decodes quoted-printable content before asserting the complete invitation body. Corrected invitation `4` superseded invitation `3`, and email job `2094` completed on its first attempt with no error.

The corrected invitation arrived intact and its link successfully opened the recipient-facing storefront page. Registration, Microsoft 365 verification delivery, email verification, sign-in, and final acceptance were then exercised successfully; verification email job `2095` completed on its first attempt. Invitation `4` was accepted at `2026-09-24T18:07:54.450Z`. Vendure customer `71` (`sjanvey@janvey.com`) is now actively linked to NetSuite customer `712`, contact `5847`, with no eligibility issue. The test invitation intentionally applied `requiresApproval=false`, `canApproveOrders=false`, and no default ship-to. The accepted state makes the invitation unavailable for reuse, matching the replay rejection already established by the earlier local end-to-end test.
