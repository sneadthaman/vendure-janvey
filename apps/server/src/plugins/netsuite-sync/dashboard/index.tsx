...
function NetsuiteCustomersPage(){
    ...
    async function loadInviteContacts(accountId:string){const sequence=++inviteLoadSequence.current;setInviteAccountId(accountId);setInviteContacts([]);setError('');if(!accountId){setLoadingInviteContacts(false);return;}const account=accounts.find(item=>item.id===accountId);if(!account)return;setLoadingInviteContacts(true);try{const result=await api.query<{inspectNetsuiteCustomer:CustomerInspection}>(inspectCustomerQuery,{customerId:account.netsuiteInternalId});if(sequence!==inviteLoadSequence.current)return;const linkedIds=new Set(account.contacts.map(contact=>contact.netsuiteContactId).filter(Boolean));setInviteContacts(result.inspectNetsuiteCustomer.contacts.filter(contact=>contact.active&&contact.emailAddress&&!linkedIds.has(contact.internalId)).sort((a,b)=>contactLabel(a).localeCompare(contactLabel(b))));}catch(e){if(sequence===inviteLoadSequence.current)setError(message(e));}finally{if(sequence===inviteLoadSequence.current)setLoadingInviteContacts(false);}}
    ...
    return <Page pageId="netsuite-customers"><PageTitle>NetSuite customer accounts</PageTitle><PageLayout><PageBlock column="main" blockId="netsuite-customer-controls">
        ...
        </form><div className="mt-4 text-sm">{invitations.slice(0,10).map(item=><p key={item.id}>{item.account.companyName} · {item.emailAddress} · {item.acceptedAt?'accepted':item.revokedAt?'revoked':new Date(item.expiresAt)<new Date()?'expired':'pending'}</p>)}</div></section>
