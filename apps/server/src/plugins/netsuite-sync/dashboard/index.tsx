...
function NetsuiteCustomersPage(){
    ...
    async function invite(event:React.FormEvent<HTMLFormElement>){event.preventDefault();const data=new FormData(event.currentTarget);setBusy(true);try{await api.mutate(inviteContactMutation,{input:{accountId:data.get('accountId'),netsuiteContactId:data.get('netsuiteContactId'),defaultShippingAddressId:String(data.get('addressId')||'')||null,requiresApproval:data.get('requiresApproval')==='on',canApproveOrders:data.get('canApproveOrders')==='on'}});await refresh();event.currentTarget.reset();}catch(e){setError(message(e));}finally{setBusy(false);}}
    ...
    return <Page pageId="netsuite-customers"><PageTitle>NetSuite customer accounts</PageTitle><PageLayout><PageBlock column="main" blockId="netsuite-customer-controls">
        ...
        <section className="mb-8 rounded border p-4"><h2 className="font-semibold mb-3">Invite a NetSuite contact</h2><p className="mb-3 text-sm">Enter the immutable contact ID shown by Inspect. The live NetSuite contact supplies the recipient name and email; invitations expire after seven days.</p><form onSubmit={invite} className="grid gap-3 md:grid-cols-2">
            <select name="accountId" required className="rounded border px-3 py-2"><option value="">NetSuite account</option>{accounts.filter(account=>account.active&&account.webCustomer).map(account=><option key={account.id} value={account.id}>{account.companyName} ({account.netsuiteInternalId})</option>)}</select>
            <input name="netsuiteContactId" required className="rounded border px-3" placeholder="NetSuite contact ID"/><input name="addressId" className="rounded border px-3" placeholder="Default local address ID (optional)"/>
            <label><input name="requiresApproval" type="checkbox"/> Requires approval</label><label><input name="canApproveOrders" type="checkbox"/> Can approve orders</label><Button disabled={busy} type="submit">Send invitation</Button>
