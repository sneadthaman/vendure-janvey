...
interface Contact {id:string;customer:Customer;netsuiteContactId:string|null;requiresApproval:boolean;canApproveOrders:boolean;active:boolean;eligibilityIssue:string|null;defaultShippingAddress:Address|null;}
interface Account {id:string;netsuiteInternalId:string;companyName:string;taxable:boolean;taxExempt:boolean;active:boolean;webCustomer:boolean;eligibilityIssue:string|null;lastSyncedAt:string|null;addresses:Address[];contacts:Contact[];}
...
const accountsQuery=`query { netsuiteAccounts { id netsuiteInternalId companyName taxable taxExempt active webCustomer eligibilityIssue lastSyncedAt addresses { id label streetLine1 city province postalCode active } contacts { id netsuiteContactId requiresApproval canApproveOrders active eligibilityIssue customer { id firstName lastName emailAddress } defaultShippingAddress { id label } } } netsuiteCustomerSyncRuns { id createdAt status netsuiteCustomerId counts issues } netsuiteApprovalOrders { id createdAt status order { id code state totalWithTax currencyCode } requester { id firstName lastName emailAddress } account { id companyName } } netsuiteContactInvitations { id createdAt emailAddress netsuiteContactId expiresAt acceptedAt revokedAt account { companyName } } }`;
...
function NetsuiteCustomersPage(){
    const [accounts,setAccounts]=useState<Account[]>([]);const [runs,setRuns]=useState<CustomerRun[]>([]);const [approvals,setApprovals]=useState<Approval[]>([]);const [invitations,setInvitations]=useState<Invitation[]>([]);
    const [customers,setCustomers]=useState<Customer[]>([]);const [netsuiteId,setNetsuiteId]=useState('');const [search,setSearch]=useState('');
    const [searchingCustomers,setSearchingCustomers]=useState(false);const [customerSearchStatus,setCustomerSearchStatus]=useState('');
    const [inviteAccountId,setInviteAccountId]=useState('');const [inviteContacts,setInviteContacts]=useState<LiveContact[]>([]);const [loadingInviteContacts,setLoadingInviteContacts]=useState(false);
    const [inviteContactStatus,setInviteContactStatus]=useState('');
    ...
    const refresh=useCallback(async()=>{try{const result=await api.query<{netsuiteAccounts:Account[];netsuiteCustomerSyncRuns:CustomerRun[];netsuiteApprovalOrders:Approval[];netsuiteContactInvitations:Invitation[]}>(accountsQuery);setAccounts(result.netsuiteAccounts);setRuns(result.netsuiteCustomerSyncRuns);setApprovals(result.netsuiteApprovalOrders);setInvitations(result.netsuiteContactInvitations);setError('');}catch(e){setError(message(e));}},[]);
    ...
    async function findCustomers(){const term=search.trim();if(!term)return;setSearchingCustomers(true);setCustomers([]);setCustomerSearchStatus('');setError('');try{const result=await api.query<{customers:{items:Customer[]}}>(findCustomersQuery,{options:{take:25,filter:{_or:[{emailAddress:{contains:term}},{firstName:{contains:term}},{lastName:{contains:term}}]}}});const linked=new Map(accounts.flatMap(account=>account.contacts.map(contact=>[contact.customer.id,account.companyName] as const)));const available=result.customers.items.filter(customer=>!linked.has(customer.id));setCustomers(available);if(result.customers.items.length===0)setCustomerSearchStatus(`No Vendure login found for "${term}". Use the invitation section below for an active NetSuite contact.`);else if(available.length===0)setCustomerSearchStatus(result.customers.items.map(customer=>`${customer.emailAddress} is already linked to ${linked.get(customer.id)}.`).join(' '));else setCustomerSearchStatus(`Found ${available.length} unlinked Vendure login${available.length===1?'':'s'}. Select one below to create a staff-managed link.`);}catch(e){setError(message(e));}finally{setSearchingCustomers(false);}}
    ...
    async function loadInviteContacts(accountId:string){const sequence=++inviteLoadSequence.current;setInviteAccountId(accountId);setInviteContacts([]);setInviteContactStatus('');setError('');if(!accountId){setLoadingInviteContacts(false);return;}const account=accounts.find(item=>item.id===accountId);if(!account)return;setLoadingInviteContacts(true);try{const result=await api.query<{inspectNetsuiteCustomer:CustomerInspection}>(inspectCustomerQuery,{customerId:account.netsuiteInternalId});if(sequence!==inviteLoadSequence.current)return;const linkedIds=new Set(account.contacts.map(contact=>contact.netsuiteContactId).filter(Boolean));const eligible=result.inspectNetsuiteCustomer.contacts.filter(contact=>contact.active&&contact.emailAddress);const available=eligible.filter(contact=>!linkedIds.has(contact.internalId)).sort((a,b)=>contactLabel(a).localeCompare(contactLabel(b)));setInviteContacts(available);const linkedCount=eligible.length-available.length;setInviteContactStatus(`Loaded ${available.length} available contact${available.length===1?'':'s'} from NetSuite${linkedCount?`; ${linkedCount} already linked and hidden`:''}.`);}catch(e){if(sequence===inviteLoadSequence.current)setError(message(e));}finally{if(sequence===inviteLoadSequence.current)setLoadingInviteContacts(false);}}
    async function invite(event:React.FormEvent<HTMLFormElement>){event.preventDefault();const form=event.currentTarget;const data=new FormData(form);setBusy(true);try{await api.mutate(inviteContactMutation,{input:{accountId:data.get('accountId'),netsuiteContactId:data.get('netsuiteContactId'),defaultShippingAddressId:String(data.get('addressId')||'')||null,requiresApproval:data.get('requiresApproval')==='on',canApproveOrders:data.get('canApproveOrders')==='on'}});await refresh();form.reset();setInviteAccountId('');setInviteContacts([]);}catch(e){setError(message(e));}finally{setBusy(false);}}
    ...
    return <Page pageId="netsuite-customers"><PageTitle>NetSuite customer accounts</PageTitle><PageLayout><PageBlock column="main" blockId="netsuite-customer-controls">
        ...
        <section className="mb-8 rounded border p-4"><h2 className="font-semibold mb-2">Link an existing Vendure login</h2><p className="mb-3 text-sm">This search checks people who already registered in Vendure. To find a contact that exists only in NetSuite, use <strong>Invite a NetSuite contact</strong> below.</p><div className="flex gap-2 mb-2"><input className="flex-1 rounded border px-3" value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();void findCustomers();}}} placeholder="Vendure login name or email"/><Button type="button" disabled={searchingCustomers||!search.trim()} onClick={()=>void findCustomers()}>{searchingCustomers?'Searching...':'Find Vendure logins'}</Button></div>{customerSearchStatus&&<p aria-live="polite" className="mb-3 text-sm">{customerSearchStatus}</p>}<form onSubmit={link} className="grid gap-3 md:grid-cols-2">
            ...
            <select name="accountId" required className="rounded border px-3 py-2"><option value="">NetSuite account</option>{accounts.filter(account=>account.active&&account.webCustomer).map(account=><option key={account.id} value={account.id}>{account.companyName} ({account.netsuiteInternalId})</option>)}</select>
        ...
        <section className="mb-8 rounded border p-4"><h2 className="font-semibold mb-3">Invite a NetSuite contact</h2><p className="mb-3 text-sm">Select an eligible account to load its active NetSuite contacts. The live contact supplies the recipient name and email; invitations expire after seven days.</p><form onSubmit={invite} className="grid gap-3 md:grid-cols-2">
            <select name="accountId" required value={inviteAccountId} onChange={event=>void loadInviteContacts(event.target.value)} className="rounded border px-3 py-2"><option value="">NetSuite account</option>{accounts.filter(account=>account.active&&account.webCustomer).map(account=><option key={account.id} value={account.id}>{account.companyName} ({account.netsuiteInternalId})</option>)}</select>
            <select name="netsuiteContactId" required disabled={!inviteAccountId||loadingInviteContacts} className="rounded border px-3 py-2"><option value="">{loadingInviteContacts?'Loading contacts...':inviteAccountId&&inviteContacts.length===0?'No available contacts with email':'NetSuite contact'}</option>{inviteContacts.map(contact=><option key={contact.internalId} value={contact.internalId}>{contactLabel(contact)} · {contact.emailAddress}</option>)}</select>
            <select name="addressId" disabled={!inviteAccountId} className="rounded border px-3 py-2"><option value="">No default ship-to</option>{accounts.find(account=>account.id===inviteAccountId)?.addresses.filter(address=>address.active).map(address=><option key={address.id} value={address.id}>{address.label||`${address.streetLine1}, ${address.city}`}</option>)}</select>
            <label><input name="requiresApproval" type="checkbox"/> Requires approval</label><label><input name="canApproveOrders" type="checkbox"/> Can approve orders</label><Button disabled={busy||loadingInviteContacts||inviteContacts.length===0} type="submit">Send invitation</Button>
        </form>{inviteContactStatus&&<p aria-live="polite" className="mt-3 text-sm">{inviteContactStatus}</p>}<div className="mt-4 text-sm">{invitations.slice(0,10).map(item=><p key={item.id}>{item.account.companyName} · {item.emailAddress} · {item.acceptedAt?'accepted':item.revokedAt?'revoked':new Date(item.expiresAt)<new Date()?'expired':'pending'}</p>)}</div></section>
        <section className="space-y-4"><h2 className="text-lg font-semibold">Accounts</h2>{accounts.map(account=><article key={account.id} className="rounded border p-4"><h3 className="font-semibold">{account.companyName} · NetSuite {account.netsuiteInternalId}</h3><p className="text-sm">Status: {account.active&&account.webCustomer?'eligible':account.eligibilityIssue||'ineligible'} · Tax: {account.taxExempt?'exempt':account.taxable?'taxable':'not taxable'} · Last sync: {account.lastSyncedAt?new Date(account.lastSyncedAt).toLocaleString():'never'}</p><p className="text-sm">Ship-to addresses: {account.addresses.filter(a=>a.active).map(a=>`${a.id}: ${a.label||a.streetLine1}, ${a.city}`).join(' · ')||'none'}</p>
            {account.contacts.map(contact=><div key={contact.id} className="mt-3 flex flex-wrap items-center gap-3 border-t pt-3"><span className="min-w-64">{contact.customer.firstName} {contact.customer.lastName} · {contact.customer.emailAddress}{contact.active?'':` · ${contact.eligibilityIssue||'inactive'}`}</span><label><input type="checkbox" checked={contact.requiresApproval} onChange={e=>void update(contact,{requiresApproval:e.target.checked})}/> Requires approval</label><label><input type="checkbox" checked={contact.canApproveOrders} onChange={e=>void update(contact,{canApproveOrders:e.target.checked})}/> Approver</label><select value={contact.defaultShippingAddress?.id||''} onChange={e=>void update(contact,{defaultShippingAddressId:e.target.value||null})} className="rounded border px-2 py-1"><option value="">No default ship-to</option>{account.addresses.filter(a=>a.active).map(address=><option key={address.id} value={address.id}>{address.label||address.streetLine1}</option>)}</select><Button variant="outline" onClick={()=>void unlink(contact)}>Unlink</Button></div>)}
        </article>)}</section>
